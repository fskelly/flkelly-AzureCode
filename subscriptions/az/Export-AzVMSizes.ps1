...
<#	Main script #>
	...
	{
		$AZAccount = Login-AzAccount
	}
	Else
	{
		$AZAccount = Login-AzAccount -Credential $Credential
	}
	...
	If ($AZAccount -ne $null)
	{
		...
		{
			$Locations = Get-AzLocation -WarningAction SilentlyContinue | Where-Object {$_.Location -eq $Location -or $_.DisplayName -eq $Location} 
		}
		Else
		{
			$Locations = Get-AzLocation -WarningAction SilentlyContinue
		}
		...
		If ($Locations.Count -ge 1)
		{
			...
			ForEach ($LocationItem in $Locations)
			{
				...
				# Get the VM sizes
				ForEach ($VMSizeItem in (Get-AzVmSize -Location $LocationItem.Location))
				{
